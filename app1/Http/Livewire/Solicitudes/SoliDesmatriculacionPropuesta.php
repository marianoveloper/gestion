...
class SoliDesmatriculacionPropuesta extends Component
{
    use WithPagination;
    ...
    public function render()
    {
       ...
       // $user->id $user->academic_id
        ...
        $desmatriculacion = DesmatriculacionPropuesta::where('user_id',$user->id)
        ->propuesta($this->propuesta_id)
        ->academic($this->academic_id)
        ->latest('id')
        ->paginate(16);

        return view('livewire.solicitudes.soli-desmatriculacion-propuesta',compact('desmatriculacion','academic','propuesta'));
    }
